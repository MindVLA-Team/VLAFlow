...
"""
vlaflow World-Model + Language (MindLWPI) Trainer

Based on train_vlaflow_wm.py but with:
  - Multi-loss monitoring (action_fm_loss, latent_pred_loss, lang_action_loss, total_loss)
  - Uses lerobot_wm_datasets for future-frame support (single VLA dataloader; the LAP
    language-action CE is derived from the action chunk inside the framework forward(),
    so no separate VLM dataloader is needed — RoboInternVQA co-training is excluded)
  - Independent from train_vlaflow_wm.py / train_vlaflow.py to ensure zero risk to
    existing pipelines
"""
...
class WMVLATrainer(TrainerUtils):
    ...
    def _train_step(self, batch_vla):
        ...
        # action_fm_loss and latent_pred_loss are provided by MindLWPI/MindWPI for monitoring
        ...
        # lang_action_loss is provided by MindLWPI (LAP action-language CE; 0 when
        # enable_lang_action=False, e.g. during downstream fine-tuning)
        if "lang_action_loss" in output_dict:
            metrics["lang_action_loss"] = output_dict["lang_action_loss"].item()
...
def main():
    parser = argparse.ArgumentParser(description="vlaflow World-Model + Language (MindLWPI) Training")
