...
# Process-level deduplication for resolution-mismatch warnings emitted by
# ``VideoTransform.check_input``. Keys are ``(video_key, declared_res, actual_res)``
# tuples so each unique mismatch warns at most once across the whole run, even
# when 800 datasets each instantiate their own VideoToTensor / VideoResize /
# VideoColorJitter triple.
_RESOLUTION_MISMATCH_WARNED: set[tuple] = set()
...
class VideoTransform(ModalityTransform):
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        for key in self.apply_to:
            split_keys = key.split(".", 1)  # split on first dot only; sub_key may itself contain dots
            assert len(split_keys) == 2, f"Invalid key: {key}. Expected format: modality.key[.subkey...]"
...
class VideoToTensor(VideoTransform):
    ...
    def check_input(self, data: dict):
        ...
        Resolution mismatch is tolerated silently — every downstream pipeline
        ends with ``VideoResize`` to a fixed target (224x224), so the
        modality.json-declared vs. actual resolution discrepancy is harmless.
        ``self._original_resolutions`` is still updated in-memory so later
        transforms see a consistent declaration.
        """
        for key in self.apply_to:
            ...
            if key in self.original_resolutions:
                actual_t = tuple(input_resolution)
                expected_t = tuple(self.original_resolutions[key])
                if actual_t != expected_t:
                    self._original_resolutions[key] = actual_t
