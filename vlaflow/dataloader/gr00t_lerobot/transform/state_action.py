...
class Normalizer:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == "q99":
            ...
            # In the case of min ≈ max (degenerate range, e.g. a near-constant
            # channel like the z-coordinate of a planar pushing task), the
            # normalization formula 2*(x-min)/(max-min)-1 explodes. Treat any
            # range <= 1e-6 as degenerate and emit 0.
            mask = (max - min) > 1e-6
            normalized = torch.zeros_like(x)
