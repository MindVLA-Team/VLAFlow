...
from vlaflow.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
...
from vlaflow.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps
...
def _read_parquet_file(parquet_path: Path) -> pd.DataFrame:
    """Helper function to read a single parquet file. Used for multiprocessing."""
    try:
...
def calculate_dataset_statistics(parquet_paths: list[Path], num_workers: int = None) -> dict:
    """Calculate the dataset statistics of all columns for a list of parquet files.

    Args:
        parquet_paths: List of paths to parquet files
        num_workers: Number of parallel workers for reading parquet files (default: min(cpu_count(), 32))
    """
    if num_workers is None:
        num_workers = min(cpu_count(), 32)

    sorted_paths = sorted(list(parquet_paths))
    print(f"[RANK 0] Loading {len(sorted_paths)} parquet files using {num_workers} workers...")

    with Pool(num_workers) as pool:
        all_low_dim_data_list = list(
            tqdm(
                pool.imap(_read_parquet_file, sorted_paths, chunksize=100),
                total=len(sorted_paths),
                desc="Collecting all parquet files",
            )
        )

    all_low_dim_data_list = [df for df in all_low_dim_data_list if df is not None]

    print(f"[RANK 0] Successfully loaded {len(all_low_dim_data_list)} files, concatenating...")
    all_low_dim_data = pd.concat(all_low_dim_data_list, axis=0)
    del all_low_dim_data_list  # Free memory

    dataset_statistics = {}
    print(f"[RANK 0] Computing statistics for {len(all_low_dim_data.columns)} modalities...")
    ...
    print("[RANK 0] Statistics calculation complete!")
...
def _build_stats_cache_config(
    ...
    return {
        # v1: legacy — action delta/rel stats overwrote the source observation column,
        #     so state and action could not be normalised differently.
        # v2: action delta/rel stats are stored under dedicated ``action.{subkey}``
        #     keys, allowing state to retain absolute statistics.
        "schema_version": 2,
        "mode": action_mode,
    }
...
def _save_stats_cache(stats_path: Path, cache_config: dict, statistics: dict) -> None:
    ...
    with open(tmp_path, "w") as f:
        ...
        f.flush()
        os.fsync(f.fileno())  # flush data to NFS server before atomic rename
    os.replace(tmp_path, stats_path)


def _expand_apply_keys_for_delta_or_rel(
    lerobot_modality_meta: "LeRobotModalityMetadata",
    parquet_paths: list[Path],
    explicit_state_map: dict[str, str] | None,
) -> tuple[list[str], dict[str, str]]:
    """Expand action_mode_apply_keys to ALL action keys defined in modality.json
    that have a valid state counterpart and a backing parquet column.

    The cache produced by ``calculate_{delta,rel}_action_statistics`` is then
    shared across multi-robot views of the same dataset (e.g. OXE_AugE
    ``austin_buds_dataset_augmented`` accessed as ur5e / sawyer / widowX / ...).
    Without this expansion, the cache would only contain whichever robot's
    apply_keys triggered the original compute, and other robots would silently
    fall back to absolute statistics → broken normalisation.

    Skips an action key if any of:
      * its parquet ``original_key`` column is missing in the dataset
      * the corresponding state key is missing or its column is missing
    """
    if not parquet_paths:
        return [], {}

    # Read just the schema (column names) of the first parquet — cheap.
    sample_cols = set(pd.read_parquet(parquet_paths[0]).columns)

    explicit_state_map = explicit_state_map or {}
    action_meta = lerobot_modality_meta.action
    state_meta = lerobot_modality_meta.state

    expanded_apply: list[str] = []
    expanded_state_map: dict[str, str] = {}
    for action_subkey, a_cfg in action_meta.items():
        a_orig = a_cfg.original_key or action_subkey
        if a_orig not in sample_cols:
            continue

        action_key = f"action.{action_subkey}"
        # Resolve state mapping: explicit override → default same-subkey
        state_key = explicit_state_map.get(action_key, f"state.{action_subkey}")
        state_subkey = state_key.replace("state.", "", 1)
        if state_subkey not in state_meta:
            continue
        s_cfg = state_meta[state_subkey]
        s_orig = s_cfg.original_key or state_subkey
        if s_orig not in sample_cols:
            continue

        expanded_apply.append(action_key)
        expanded_state_map[action_key] = state_key

    return expanded_apply, expanded_state_map
...
def _compute_statistics_for_mode(
    ...
    # Expand apply_keys to all action keys with valid state counterpart and a
    # backing parquet column. This makes the cache shared across multi-robot
    # views of the same dataset (e.g. OXE_AugE austin_buds_dataset_augmented
    # accessed as ur5e / sawyer / widowX / ...).
    expanded_apply, expanded_state_map = _expand_apply_keys_for_delta_or_rel(
        lerobot_modality_meta=lerobot_modality_meta,
        parquet_paths=parquet_paths,
        explicit_state_map=action_mode_state_map,
    )
    if not expanded_apply:
        # Fall back to the user-supplied keys if the dataset doesn't seem to
        # provide any matching columns (shouldn't happen in practice; the
        # downstream functions will surface a clearer error).
        expanded_apply = action_mode_apply_keys
        expanded_state_map = action_mode_state_map
        expanded_action_keys = action_keys_full
        expanded_state_keys = state_keys_full
    else:
        # ``_get_action_col_slices`` validates each apply key + state target
        # against ``action_keys_full`` / ``state_keys_full``. Extend both sets
        # with the cross-robot keys surfaced by the expansion so the validator
        # does not reject e.g. ``state.widowX.ee_pos`` when the user's runtime
        # config is sawyer-only.
        expanded_action_keys = list(action_keys_full)
        for k in expanded_apply:
            if k not in expanded_action_keys:
                expanded_action_keys.append(k)
        expanded_state_keys = list(state_keys_full)
        for k in expanded_state_map.values():
            if k not in expanded_state_keys:
                expanded_state_keys.append(k)

    if action_mode == "delta":
        return calculate_delta_action_statistics(
            parquet_paths=parquet_paths,
            lerobot_modality_meta=lerobot_modality_meta,
            action_keys_full=expanded_action_keys,
            state_keys_full=expanded_state_keys,
            action_indices=action_indices,
            state_indices=state_indices,
            action_mode_apply_keys=expanded_apply,
            action_mode_state_map=expanded_state_map,
...
def _load_or_compute_statistics(
    ...
    # Validate that the cached statistics cover every action key currently
    # requested for delta / rel normalisation. If the cache was originally
    # written by a sibling robot type whose apply_keys form a subset, the
    # current robot's `action.{subkey}` entries will be missing and the
    # transform layer would silently fall back to absolute statistics.
    if le_statistics is not None and action_mode in ("delta", "rel"):
        required_apply = _normalize_action_mode_apply_keys(action_mode_apply_keys, action_keys_full)
        # Exclude apply_keys whose backing parquet column does not exist for
        # this dataset — they cannot be computed even by a fresh recompute, so
        # treating them as "missing" would force an infinite recompute loop on
        # every subsequent load. Determine availability by reusing the same
        # expansion helper that the recompute path calls.
        available_apply, _ = _expand_apply_keys_for_delta_or_rel(
            lerobot_modality_meta=lerobot_modality_meta,
            parquet_paths=parquet_paths,
            explicit_state_map=action_mode_state_map,
        )
        available_set = set(available_apply)
        missing = [k for k in required_apply if k in available_set and k not in le_statistics]
        if missing:
            print(
                f"[stats] Cache at {stats_path} missing action keys for current "
                f"robot ({len(missing)} keys, e.g. {missing[:3]}); recomputing "
                f"with all-robot expansion."
            )
            le_statistics = None
...
def _get_action_col_slices(
    ...
    """Build mapping ``read_col -> [slice_descriptor, ...]`` for delta / rel statistics.

    Each slice descriptor is a dict with keys:
      - ``action_subkey``  — e.g. ``"widowX.ee_pos"`` (used as the *write* target
        ``action.{action_subkey}`` so action stats no longer collide with state).
      - ``action_slice`` / ``state_slice`` — ``(start, end)`` index ranges.
      - ``state_col`` — LeRobot column to read state from.
      - ``action_padding`` / ``state_padding`` — padding strategy.
    """
    ...
    # Build per-column mapping: action column -> list of slice descriptor dicts
    action_col_slices: dict[str, list[dict]] = {}
...
def calculate_delta_action_statistics(
    ...
    # Per-action-subkey accumulator: each entry is a [T_chunk, slice_width] delta array.
    # Storing per-subkey (instead of per read column) means action stats no longer
    # overwrite the absolute statistics of the source column shared with state.
    delta_accum: dict[str, list[np.ndarray]] = {}
    for parquet_path in tqdm(sorted(list(parquet_paths)), desc="Collecting delta action stats"):
        ...
        for action_col, slice_list in action_col_slices.items():
            ...
            prepared = []
            for desc in slice_list:
                state_col = desc["state_col"]
                ...
                ss = desc["state_slice"]
                state_part_full = state_matrix[:, ss[0] : ss[1]]
                prepared.append((desc, state_part_full))
            for base_index in range(trajectory_length):
                action_steps = np.array(action_indices) + base_index
                state_steps = np.array(state_indices) + base_index
                for desc, state_part_full in prepared:
                    a_slice = desc["action_slice"]
                    action_chunk = _get_chunk(action_matrix, action_steps, desc["action_padding"])
                    action_part_chunk = action_chunk[:, a_slice[0] : a_slice[1]]
                    state_chunk = _get_chunk(state_part_full, state_steps, desc["state_padding"])
                    ...
                    delta_accum.setdefault(desc["action_subkey"], []).append(out)

    delta_stats = copy.deepcopy(base_stats)
    for action_subkey, series_list in delta_accum.items():
        if not series_list:
            continue
        all_values = np.concatenate(series_list, axis=0).astype(np.float32)
        delta_stats[f"action.{action_subkey}"] = {
...
def calculate_rel_action_statistics(
    ...
    # Per-action-subkey accumulator: each entry is a [T_chunk, slice_width] rel array.
        ...
        for action_col, slice_list in action_col_slices.items():
            ...
            for base_index in range(trajectory_length):
                ...
                for desc, state_part_full in prepared:
                    ...
                    rel_accum.setdefault(desc["action_subkey"], []).append(out)

    rel_stats = copy.deepcopy(base_stats)
    for action_subkey, series_list in rel_accum.items():
        if not series_list:
            continue
        all_values = np.concatenate(series_list, axis=0).astype(np.float32)
        rel_stats[f"action.{action_subkey}"] = {
            "mean": np.mean(all_values, axis=0).tolist(),
            "std": np.std(all_values, axis=0).tolist(),
            "min": np.min(all_values, axis=0).tolist(),
            "max": np.max(all_values, axis=0).tolist(),
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Detect lerobot version: auto-read from meta/info.json, data_cfg only as override.
        _explicit_version = self.data_cfg.get("lerobot_version") if self.data_cfg else None
        if _explicit_version is not None:
            self._lerobot_version = _explicit_version
        else:
            _info_path = Path(dataset_path) / "meta" / "info.json"
            if _info_path.exists():
                with open(_info_path) as _f:
                    _cv = json.load(_f).get("codebase_version", "v2.0")
                # v2.x variants all use the same v2.0 code path
                self._lerobot_version = "v2.0" if _cv.startswith("v2") else _cv
            else:
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        for modality in ["state", "action"]:
            ...
            for subkey in le_state_action_meta:
                ...
                if np.issubdtype(state_action_dtype, np.floating):
                    ...
                    continuous = False
                _raw_dim = le_state_action_meta[subkey].end - le_state_action_meta[subkey].start
                _rot_type = le_state_action_meta[subkey].rotation_type
                # QUATERNION rotation keys are converted to 3D Euler XYZ at load time, so
                # report shape=[3] to downstream transforms (e.g. ConcatTransform dim check).
                _shape_dim = 3 if _rot_type == RotationType.QUATERNION else _raw_dim
                simplified_modality_meta[modality][subkey] = {
                    "absolute": le_state_action_meta[subkey].absolute,
                    "rotation_type": _rot_type,
                    "shape": [_shape_dim],
        ...
        for new_key in le_modality_meta.video:
            ...
            le_video_meta = le_info["features"].get(original_key)
            if le_video_meta is None:
                # Some datasets in a comprehensive modality.json may not have all video
                # keys (e.g. OXE_AugE datasets without every robot's camera).  Skip
                # missing keys — the DataConfig's modality_config should only reference
                # cameras that are actually present.
                import warnings

                warnings.warn(
                    f"Video key '{original_key}' not found in info.json features for "
                    f"{self.dataset_path}; skipping this video modality.",
                    stacklevel=2,
                )
                continue
            _names = le_video_meta.get("names")
            if _names is not None:
                height = le_video_meta["shape"][_names.index("height")]
                width = le_video_meta["shape"][_names.index("width")]
            else:
                # Fallback when 'names' is absent (e.g. OXE_AugE info.json).
                # Shape is either [H, W, C] or [C, H, W]; detect by whether the
                # first dimension looks like a channel count (≤ 4).
                _shape = le_video_meta["shape"]
                if len(_shape) >= 3 and _shape[0] <= 4:  # [C, H, W]
                    height, width = _shape[1], _shape[2]
                else:  # [H, W, C]
        ...
        if le_statistics is None:
            # Non-main ranks read the stats file written by rank 0.
            # NFS attribute/directory caches can be stale immediately after
            # dist.barrier() — retry with exponential backoff while busting the
            # NFS directory entry cache on each attempt.
            import time as _time

            for _attempt in range(6):
                try:
                    # Force NFS client to re-read the directory listing, which
                    # invalidates stale "file not found" attribute cache entries.
                    os.listdir(stats_path.parent)
                except OSError:
                    pass
                le_statistics = _load_stats_cache(
                    stats_path,
                ...
                if le_statistics is not None:
                    break
                _time.sleep(min(0.5 * (2**_attempt), 8.0))  # 0.5 1 2 4 8 8 s
            if le_statistics is None:
        ...
        for our_modality in ["state", "action"]:
            ...
            for subkey in simplified_modality_meta[our_modality]:
                ...
                _is_quat = state_action_meta.rotation_type == RotationType.QUATERNION
                if _is_quat:
                    # Quaternion keys are converted to 3D Euler XYZ at load time.
                    # The original quaternion statistics are meaningless post-conversion,
                    # so synthesize plausible Euler-angle statistics (range [-π, π]).
                    _pi = float(np.pi)
                ...
                # Action modality (delta / rel mode): prefer dedicated per-subkey
                # statistics written by calculate_{delta,rel}_action_statistics under
                # ``action.{subkey}``. This avoids the legacy collision where state
                # and action shared the same source column and could not be
                # normalised independently.
                if our_modality == "action":
                    action_full_key = f"action.{subkey}"
                    if action_full_key in le_statistics:
                        dataset_statistics["action"][subkey] = dict(le_statistics[action_full_key])
                        continue

                le_modality = state_action_meta.original_key
                if le_modality not in le_statistics:
                    # Some datasets built from a comprehensive modality.json (e.g. OXE_AugE)
                    # may lack statistics for robot arms not present in this specific split.
                    # Skip the key and remove it from simplified_modality_meta to keep both
                    # structures consistent.
                    import warnings

                    warnings.warn(
                        f"Statistics key '{le_modality}' not found in dataset statistics for "
                        f"{self.dataset_path}; skipping this {our_modality} key '{subkey}'.",
                        stacklevel=2,
                    )
            ...
            # Remove keys that had no statistics so simplified_modality_meta stays in sync.
            for subkey in missing_subkeys:
                del simplified_modality_meta[our_modality][subkey]
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        # ---------- only build by rank0  ----------
        if is_main():
            num_workers = int(os.environ.get("VLAFLOW_STEPS_WORKERS", "16"))
            if num_workers > 1:
                all_steps = self._get_all_steps_parallel(num_workers=num_workers)
            else:
    ...
    def _get_all_steps_parallel(self, num_workers: int = 16) -> list[tuple[int, int]]:
        """Parallel implementation of step-index building.

        Instead of loading the full trajectory parquet just to verify the language
        instruction, this method reads **only the required annotation column** from
        each parquet file and parallelises I/O across threads.

        For v3.0 datasets (multiple episodes per chunk file) trajectories are first
        grouped by (chunk_index, file_index) so each chunk file is read at most once.
        """
        from collections import defaultdict
        from concurrent.futures import ThreadPoolExecutor, as_completed

        # Identify the original parquet column name for the language key (if any).
        has_language_modality = "language" in self.modality_keys and len(self.modality_keys["language"]) > 0
        lang_orig_key: str | None = None
        if has_language_modality:
            lang_key = self.modality_keys["language"][0]
            subkey = lang_key.replace("annotation.", "", 1)
            ann_meta = self.lerobot_modality_meta.annotation
            if ann_meta and subkey in ann_meta and ann_meta[subkey].original_key:
                lang_orig_key = ann_meta[subkey].original_key
            else:
                lang_orig_key = subkey  # fallback: use the subkey directly

        # ------------------------------------------------------------------
        # Worker functions
        # ------------------------------------------------------------------
        tasks_df = self.tasks  # reference for task_index lookup (v2.0)

        def _check_valid_v2(traj_id: int) -> bool:
            """v2.0: each trajectory has its own parquet — read minimal columns."""
            try:
                chunk_index = traj_id // self.chunk_size
                parquet_path = self.dataset_path / self.data_path_pattern.format(
                    episode_chunk=chunk_index, episode_index=traj_id
                )
                if not lang_orig_key:
                    return parquet_path.exists()
                read_cols = [lang_orig_key]
                df = pd.read_parquet(parquet_path, columns=read_cols)
                val = df[lang_orig_key].iloc[0]
                if isinstance(val, str):
                    return bool(val.strip())
                # Integer task_index — look up the tasks table.
                try:
                    task_str = tasks_df.loc[int(val)]["task"]
                    return bool(str(task_str).strip())
                except Exception:
                    return True
            except Exception as e:
                print(f"[_get_all_steps] Skipping traj {traj_id}: {e}")
                return False

        def _check_valid_v3_chunk(parquet_path: Path, traj_ids: list[int]) -> dict[int, bool]:
            """v3.0: read one chunk file with minimal columns, validate all episodes in it."""
            results: dict[int, bool] = {}
            try:
                read_cols = ["episode_index"]
                if lang_orig_key:
                    read_cols.append(lang_orig_key)
                df = pd.read_parquet(parquet_path, columns=read_cols)
                for traj_id in traj_ids:
                    sub = df.loc[df["episode_index"] == traj_id]
                    if sub.empty:
                        results[traj_id] = False
                        continue
                    if not lang_orig_key:
                        results[traj_id] = True
                        continue
                    val = sub[lang_orig_key].iloc[0]
                    if isinstance(val, str):
                        results[traj_id] = bool(val.strip())
                    else:
                        try:
                            task_str = tasks_df.loc[int(val)]["task"]
                            results[traj_id] = bool(str(task_str).strip())
                        except Exception:
                            results[traj_id] = True
            except Exception as e:
                print(f"[_get_all_steps] Failed to read chunk {parquet_path}: {e}")
                for traj_id in traj_ids:
                    results[traj_id] = False
            return results

        # ------------------------------------------------------------------
        # Build valid_map in parallel
        # ------------------------------------------------------------------
        valid_map: dict[int, bool] = {}

        if self._lerobot_version == "v3.0":
            # Group trajectories by their (chunk_index, file_index) — share reads.
            chunk_groups: dict[tuple[int, int], list[int]] = defaultdict(list)
            for traj_id in self.trajectory_ids:
                ep_meta = self.trajectory_ids_to_metadata[traj_id]
                ci = ep_meta["data/chunk_index"]
                fi = ep_meta["data/file_index"]
                chunk_groups[(ci, fi)].append(traj_id)

            def _v3_worker(chunk_key: tuple[int, int]) -> dict[int, bool]:
                ci, fi = chunk_key
                parquet_path = self.dataset_path / self.data_path_pattern.format(chunk_index=ci, file_index=fi)
                return _check_valid_v3_chunk(parquet_path, chunk_groups[chunk_key])

            effective_workers = min(num_workers, len(chunk_groups))
            with ThreadPoolExecutor(max_workers=effective_workers) as executor:
                futures = {executor.submit(_v3_worker, ck): ck for ck in chunk_groups}
                for future in tqdm(
                    as_completed(futures),
                    total=len(futures),
                    desc="Getting All Steps",
                ):
                    valid_map.update(future.result())
        else:
            # v2.0: one parquet file per trajectory — fully independent.
            effective_workers = min(num_workers, len(self.trajectory_ids))
            with ThreadPoolExecutor(max_workers=effective_workers) as executor:
                futures = {executor.submit(_check_valid_v2, traj_id): traj_id for traj_id in self.trajectory_ids}
                for future in tqdm(
                    as_completed(futures),
                    total=len(futures),
                    desc="Getting All Steps",
                    ...
                    valid_map[traj_id] = future.result()

        # ------------------------------------------------------------------
        # Build all_steps in original trajectory order
        # ------------------------------------------------------------------
        all_steps: list[tuple[int, int]] = []
        skipped = 0
        for traj_id, traj_length in zip(self.trajectory_ids, self.trajectory_lengths):
            if not valid_map.get(int(traj_id), True):
                skipped += 1
                continue
            for base_index in range(int(traj_length)):
                all_steps.append((int(traj_id), base_index))

        valid_count = len(self.trajectory_ids) - skipped
        print(
            f"Parallel summary: {valid_count} valid, {skipped} skipped. "
            f"Total steps: {len(all_steps)} from {len(self.trajectory_ids)} trajectories"
        )
        return all_steps
    ...
    def _quat_wxyz_to_euler_xyz(quat_wxyz: np.ndarray) -> np.ndarray:
        """Convert quaternion (wxyz convention) to Euler XYZ angles in radians.

        Args:
            quat_wxyz: (..., 4) array with quaternion in [w, x, y, z] order.

        Returns:
            (..., 3) array of Euler XYZ angles in radians.
        """
        from scipy.spatial.transform import Rotation

        orig_shape = quat_wxyz.shape
        flat = quat_wxyz.reshape(-1, 4).astype(np.float64, copy=True)
        # Replace zero-norm quaternions (e.g. introduced by zero-padding at
        # trajectory boundaries) with the identity quaternion [w=1, x=y=z=0]
        # so scipy's `Rotation.from_quat` does not raise.
        norms = np.linalg.norm(flat, axis=1)
        zero_mask = norms < 1e-8
        if zero_mask.any():
            flat[zero_mask] = np.array([1.0, 0.0, 0.0, 0.0])
        # scipy expects [x, y, z, w] (xyzw) — reorder from wxyz
        quat_xyzw = flat[:, [1, 2, 3, 0]]
        euler = Rotation.from_quat(quat_xyzw).as_euler("xyz", degrees=False)
        return euler.reshape(*orig_shape[:-1], 3)
    ...
    def _apply_action_mode(self, data: dict) -> dict:
        ...
        for action_key in action_keys:
            ...
            # Pre-convert QUATERNION rotation keys (4D wxyz) to 3D Euler XYZ before
            # computing delta.  Both action and corresponding state must be converted.
            _action_subkey = action_key.split(".", 1)[1]
            _action_meta = self.metadata.modalities.action.get(_action_subkey)
            if _action_meta is not None and _action_meta.rotation_type == RotationType.QUATERNION:
                action_values = self._quat_wxyz_to_euler_xyz(action_values)
                state_values = self._quat_wxyz_to_euler_xyz(state_values)
                # Write back converted state so downstream transforms see 3D values.
                data[state_key] = state_values
    ...
    def _pack_sample(self, data: dict) -> dict:
        ...
        Supports two pipeline styles:
        - Pre-concat: individual keys like ``"video.cam_rgb"`` still present.
        - Post-concat: ``ConcatTransform`` has merged them into ``"video"`` /
          ``"action"`` / ``"state"`` tensors (shape ``[T, V, H, W, C]`` /
          ``[T, D]``).
        """
        prim_images = []
        wrist_views = []
        ...
        if "video" in data:
            # Post-ConcatTransform: data["video"] is [T, V, H, W, C]
            vid_tensor = np.asarray(data["video"])  # [T, V, H, W, C]
            for v_idx, video_key in enumerate(self.modality_keys["video"]):
                if v_idx >= vid_tensor.shape[-4]:
                    break
                image = vid_tensor[0, v_idx]  # [H, W, C]
        ...
        if "action" in data:
            # Post-ConcatTransform: data["action"] is [T, D]
            action = np.asarray(data["action"]).astype(np.float16)
        else:
            action_parts = []
            for action_key in self.modality_keys["action"]:
                action_parts.append(data[action_key])
            action = np.concatenate(action_parts, axis=1).astype(np.float16)

        # Catch dataset-wiring mistakes early: the pretraining mixture assumes
        # every sample emits a 14-D dual-arm action tensor. Any deviation (e.g.
        # a DataConfig that forgot SingleArmToDualArmTransform, or a mismatch
        # between ``action_concat_order`` and ``modality.json``) will cascade
        # into an opaque ``np.array(actions)`` shape error at framework loss
        # time — this assert pinpoints the offending dataset + action width.
        if action.ndim != 2 or action.shape[-1] != 14:
            raise AssertionError(
                f"[_pack_sample] Dataset '{self.dataset_name}' (tag={self.tag}) "
                f"produced action of shape {tuple(action.shape)} — expected "
                f"[T, 14]. Check its DataConfig.transform() pipeline ends with "
                f"ConcatTransform + SingleArmToDualArmTransform (for single-arm "
                f"sources) or that ConcatTransform action_concat_order sums to "
                f"14 (for native dual-arm sources)."
            )

        # Loss mask for ``action`` — 1.0 where the supervision target is real,
        # 0.0 where ``SingleArmToDualArmTransform`` padded zeros / inserted a
        # gripper sentinel. For datasets that natively output 14-D dual-arm
        # actions (e.g. RoboCOIN) the transform isn't in the pipeline, so we
        # fall back to an all-ones mask. This single `action_mask` field lets
        # frameworks compute a mixture-safe masked MSE / flow-matching loss
        # without re-deriving validity from arm-specific knowledge.
        if "action_mask" in data:
            action_mask = np.asarray(data["action_mask"]).astype(np.float16)
        else:
            action_mask = np.ones_like(action, dtype=np.float16)
        ...
        if self.data_cfg is not None and self.data_cfg.get("include_state", False) not in ["False", False]:
            if "state" in data:
                state = np.asarray(data["state"]).astype(np.float16)
            else:
                state_parts = []
    ...
    def get_language(
        ...
        for i in range(len(step_indices)):  #
            ...
            if isinstance(value, str):
                # Column stores the instruction text directly (e.g. OXE_AugE natural_language_instruction)
                task_indices.append(value)
            elif isinstance(value, (int, float)):
                task_indices.append(int(value))
            elif isinstance(value, np.ndarray) and value.dtype.kind in ("i", "u") and value.ndim == 1:
                # ASCII bytecode array (e.g. Google language_table_*: (512,) int32 zero-padded).
                # Decode printable ASCII bytes, stopping at the first NUL.
                chars = [chr(int(c)) for c in value if 0 < int(c) < 128]
                task_indices.append("".join(chars))
            else:
                task_indices.append(value.item())

        # If we collected raw strings, return them directly; otherwise look up the tasks table.
...
class LeRobotMixtureDataset(Dataset):
    ...
    def compute_overall_statistics(
        ...
        # Use the union of all keys across all datasets in this group.
        # Datasets in a heterogeneous tag group (e.g. NEW_EMBODIMENT covering multiple
        # joint-space schemas) may have different subkey sets.  Each key's statistics
        # are computed only from the datasets that actually possess that key; their
        # weights are renormalized to sum to 1 for that subset.
        modality_keys: set[str] = set()
        for ts in per_task_stats:
            modality_keys.update(ts.keys())

        for modality in sorted(modality_keys):
            # Select only the datasets that have this subkey
            relevant = [(i, ts[modality]) for i, ts in enumerate(per_task_stats) if modality in ts]
            if not relevant:
                continue

            rel_indices = np.array([i for i, _ in relevant])
            rel_weights = normalized_weights[rel_indices]
            rel_weights = rel_weights / rel_weights.sum()  # renormalize to 1

            num_dims = len(relevant[0][1]["mean"])
            ...
            for j, (_, stats) in enumerate(relevant):
                w_i = rel_weights[j]
            ...
            # Compute overall variance and std (clamp to avoid sqrt of tiny negatives)
            ...
            if percentile_mixing_method == "weighted_average":
                weighted_q01 = np.average(q01_array, axis=0, weights=rel_weights).tolist()
                weighted_q99 = np.average(q99_array, axis=0, weights=rel_weights).tolist()
    ...
    def merge_metadata(
        ...
        for modality, configs in modality_configs.items():
            if modality == "video":
                # Video configs can differ across datasets in a heterogeneous mixture
                # (different cameras, FPS, resolutions). Merge by taking the union of
                # all video keys. set_transforms_metadata() guards with `if key in
                # all_video_keys`, so extra keys are safely ignored.
                merged_video: dict = {}
                for cfg_str in configs:
                    merged_video.update(json.loads(cfg_str))
                merged_metadata["modalities"][modality] = merged_video
            else:
                # state / action modality configs must ideally be identical within a tag
                # group, but heterogeneous embodiment mixtures (e.g. NEW_EMBODIMENT
                # covering multiple joint-space schemas) may differ.  Take the union so
                # each dataset can still find its own subkeys; extra keys are ignored by
                # each dataset's transform (it only accesses its own apply_to keys).
                if len(configs) > 1:
                    merged: dict = {}
                    for cfg_str in configs:
                        merged.update(json.loads(cfg_str))
                    merged_metadata["modalities"][modality] = merged
                else:
