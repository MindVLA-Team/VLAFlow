...
def read_mode_config(pretrained_checkpoint):
    """Re-export from share_tools — canonical version lives in vlaflow.model.framework.share_tools."""
    from vlaflow.model.framework.share_tools import read_mode_config as _read_mode_config
...
class FrameworkTools:
    ...
    ``model.unnormalize_actions(...)`` keep working, but new code should prefer::
        ...
        from vlaflow.model.tools import FrameworkTools
        actions = FrameworkTools.unnormalize_actions(norm_actions, stats)
    """

    @staticmethod
    def check_unnorm_key(norm_stats: dict, unnorm_key: Optional[str]) -> str:
    ...
    @staticmethod
    def get_action_stats(norm_stats: dict, unnorm_key: Optional[str] = None) -> dict:
    ...
    def unnormalize_actions(
        ...
        if 0 <= gripper_channel_idx < normalized_actions.shape[-1]:
            normalized_actions[:, gripper_channel_idx] = np.where(normalized_actions[:, gripper_channel_idx] < 0.5, 0, 1)
        actions = np.where(
            mask,
