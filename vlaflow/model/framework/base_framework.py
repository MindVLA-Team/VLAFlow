...
from vlaflow.model.framework.share_tools import dict_to_namespace, read_mode_config
        ...
        if is_pkg:
            ...
            for _, sub_name, _ in pkgutil.iter_modules([str(sub_dir)]):
                if sub_name.startswith("_"):
                    continue
                importlib.import_module(f"vlaflow.model.framework.{module_name}.{sub_name}")
        else:
            importlib.import_module(f"vlaflow.model.framework.{module_name}")
