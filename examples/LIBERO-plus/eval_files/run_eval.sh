...
for suite in ${TASK_SUITES:-libero_spatial libero_object libero_goal libero_10}; do
    "${SIM_PYTHON:-python}" "${REPO_ROOT}/examples/LIBERO-plus/eval_files/eval_libero.py" \
        ...
        --args.num-trials-per-task "${TRIALS_PER_TASK:-1}" \
        --args.output-dir "${OUTPUT_ROOT:-${REPO_ROOT}/outputs}/LIBERO-plus" "$@"
done
